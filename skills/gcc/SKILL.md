...
```bash
onecontext commit --title "Implement JWT auth" \
...
```bash
onecontext context                          # project overview + branches
onecontext context --branch main            # branch purpose + commits
onecontext context --commit 0               # latest commit detail
onecontext context --log --limit 20         # execution trace
onecontext context --metadata file_structure # project files
```
...
```bash
onecontext update-main --section "Active Decisions" \
...
1. Run `onecontext context --log --limit 50` to understand what happened
2. Run `onecontext update-main --section "Purpose" --content "..."` with a real project description
...
1. **Start session** → `onecontext context` to orient
2. **Before experiment** → `onecontext branch create ...`
3. **After milestone** → `onecontext commit --title ... --notes ...`
4. **Experiment worked** → `onecontext merge ...`
5. **Curate** → `onecontext update-main ...` when sections are stale
...
```bash
$ONECONTEXT_REPO/scripts/run_backfill.sh /path/to/workspace --dry-run   # preview
$ONECONTEXT_REPO/scripts/run_backfill.sh /path/to/workspace --branch main  # import
```
