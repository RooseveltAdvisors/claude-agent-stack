---
name: AgentBrowser
description: Automates browser interactions for web testing, form filling, screenshots, and data extraction. USE WHEN user needs to navigate websites OR interact with web pages OR fill forms OR take screenshots OR test web applications OR extract information from web pages.
...
> **⚠️ IMPORTANT: Always close the browser when done!**
> Run `agent-browser close` at the end of every browser session. Failure to close leaves orphaned Chrome processes that consume system resources.
...
5. **Close browser: `agent-browser close`** - ALWAYS do this when finished!
...
**Tip**: Use `eval` as fallback when ref-based clicks fail on dynamic pages:
```bash
...
|-----------|-------------|
| [references/e2e-testing-patterns.md](references/e2e-testing-patterns.md) | **E2E testing best practices, fallbacks, form filling patterns** |
| [references/installation.md](references/installation.md) | Installation, platforms, architecture |
| [references/cloud-providers.md](references/cloud-providers.md) | Browserbase, Browser Use, Kernel, CDP mode |
| [references/streaming.md](references/streaming.md) | WebSocket streaming, live preview, pair browsing |
