...
function validateCommand(command: string): { allowed: boolean; message?: string; action?: string } {
  ...
  // Check if command targets a trusted endpoint (bypass exfiltration checks)
  const isTrustedEndpoint = TRUSTED_ENDPOINTS.some(endpoint => command.includes(endpoint));

  // Check each tier
  for (const [tierName, tier] of Object.entries(ATTACK_PATTERNS)) {
    // Skip exfiltration checks for trusted endpoints
    if (tierName === 'exfiltration' && isTrustedEndpoint) {
      continue;
    }
